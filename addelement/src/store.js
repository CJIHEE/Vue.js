...
export const store = new Vuex.Store({
  state: {
    currentUserid: "",
    userList : [ ],
    diaryList : [  {id:1, name:"",  date :"기본값", contents:"기본값", subject:"기본값" },
                   {id:1, name:"ㄴㄴ",  date :"ㄴㄴ", contents:"기본값", subject:"기본값" },
                   {id:1, name:"ㄴㄴ",  date :"ㄴㄴ", contents:"기본값", subject:"기본값" }
                ],     
    indexid : 0,               
  ...
  getters:{
    ...
    getMaxId : state => {
        let array = []
        for(var i = 0; i < state.diaryList.length; i++) {
            let id = state.diaryList[i].id
            array.push(id)
        }
        let maxId = Math.max.apply(null, array)
      return maxId
    }
   },
  mutations:{
